...
public class BookingCancellationTransaction {
    ...
    public BookingOrder cancelOne(
        ...
        Instant operationTime = Objects.requireNonNull(
                cancelledAt,
                "cancelledAt must not be null"
        );

        BookingOrder order = bookingOrderRepository
                .findById(validatedId)
                .orElseThrow(BookingCancellationConflictException::new);

        if (!order.userId().equals(validatedUserId)) {
            throw new BookingCancellationConflictException();
        }

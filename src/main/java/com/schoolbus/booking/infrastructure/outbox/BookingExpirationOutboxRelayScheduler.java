...
public class BookingExpirationOutboxRelayScheduler {

    private static final Logger log = LoggerFactory.getLogger(
            BookingExpirationOutboxRelayScheduler.class
    );

    private final BookingExpirationOutboxRelay relay;

    public BookingExpirationOutboxRelayScheduler(
            BookingExpirationOutboxRelay relay
    ) {
    ...
    public void relay() {
        try {
            ...
            if (result.claimed() > 0) {
                log.info(
                        "Booking expiration outbox relay completed: "
            ...
            log.error(
                    "Booking expiration outbox relay scan failed",
                    exception
            );
        }
    }

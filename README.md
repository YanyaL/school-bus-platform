...
- **学生端 SSO 第二阶段（当前分支）**：在 IAM OIDC 授权服务器之上完成 Vue 学生端 Authorization Code + PKCE 发起与回调闭环，使用 `oidc-client-ts` 管理 Discovery、state、nonce 和 PKCE；SSO 与旧 JSON 登录并行以支持渐进迁移。公共 SPA 不签发 Refresh Token，当前退出仅清理应用会话；管理端前端、BFF 与跨系统统一登出仍是后续工作（见 `docs/21-iam-sso-authorization-server.md`、`docs/22-student-sso-frontend.md`）

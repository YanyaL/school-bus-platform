...
- **IAM SSO 第一阶段（当前分支）**：OIDC 授权服务器、两个 PKCE 公共客户端、现有账户认证适配和兼容既有资源服务的 JWT Claims 已完成；公共 SPA 按规范不签发 Refresh Token，前端回调、BFF/刷新策略与统一登出仍是后续工作（见 `docs/21-iam-sso-authorization-server.md`）
